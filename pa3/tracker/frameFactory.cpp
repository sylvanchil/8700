...
FrameFactory::~FrameFactory() {
	...
	std::map<std::string, Frame*>::const_iterator fi; 
	for(fi = frames.begin(); fi != frames.end(); fi++) 
	{
		delete fi->second;
	}

	std::map<std::string, SDL_Surface*>::const_iterator si; 
	for(si = surfaces.begin(); si != surfaces.end(); si++) 
	{
		SDL_FreeSurface(si->second);
	}

	std::map<std::string, std::vector<Frame*> >::const_iterator mfi; 
	for( mfi = multiFrames.begin(); mfi != multiFrames.end(); mfi++)
	{
		for(unsigned i = 0; i < mfi->second.size(); i++)
		{
			delete mfi->second[i];
		}
	}

	std::map<std::string, std::vector<SDL_Surface*> >::const_iterator msi;
	for( msi = multiSurfaces.begin(); msi != multiSurfaces.end(); msi++)
	{
		for(unsigned i = 0; i < msi->second.size(); i++)
		{
			SDL_FreeSurface(msi->second[i]);
		}
	}
